...
app.service(
    ...
    function setupService() {
        ...
        function endRequest(httpM) {
            httpMethod = normalizedHttpMethod(httpM.method);
             if(httpM.url.indexOf('.html')==-1){
                pending.all--;
                pending[httpMethod]--;
...
app.service('interceptHttp', ['$q', 'trafficCop', function($q, trafficCop) {
    ...
    return {
        request: function(config) {
            trafficCop.startRequest(config);
            return (config);
        },
